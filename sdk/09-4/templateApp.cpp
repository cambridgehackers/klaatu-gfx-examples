...
OBJ *obj = NULL;
vec3 eye, next_eye, up = { 0.0f, 0.0f, 1.0f };
...
float rotx = 40.0f, rotz = 0.0f, distance = 5.0f;
vec2 accelerometer = { 0.0f, 0.0f }, next_accelerometer = {
0.0f, 0.0f};

float ball_speed = 6.7f, sensitivity = 3.0f;
unsigned char game_state = 0;
float game_time = 0.0f, gem_factor = 20.0f;
...
void program_draw(void *ptr)
{
	...
	if (!program->uniform_array[i].constant && !strcmp(program->uniform_array[i].name, "DIFFUSE")) {
	    ...
	    glUniformMatrix4fv(program->uniform_array[i].location, 1, GL_FALSE, (float *)GFX_get_texture_matrix());
	    GFX_pop_matrix();
...
class ClosestNotMeRayResultCallback:public btCollisionWorld::ClosestRayResultCallback {
 public:
    ClosestNotMeRayResultCallback(btRigidBody * rb, const btVector3 & p1, const btVector3 & p2):btCollisionWorld::ClosestRayResultCallback(p1, p2) {
	m_btRigidBody = rb;
    } virtual btScalar addSingleResult(btCollisionWorld::LocalRayResult & localray, bool normalinworldspace) {
			    ...
			    const btCollisionObject * btcollisionobject0, int part_0, int index_0, const btCollisionObject * btcollisionobject1, int part_1, int index_1)
{
    ...
    if ((strstr(objmesh0->name, "level_clear") || strstr(objmesh1->name, "level_clear")))
	game_state = 1;
    else if ((strstr(objmesh0->name, "player") || strstr(objmesh1->name, "player"))
	     && (strstr(objmesh0->name, "gem") || strstr(objmesh1->name, "gem"))) {
	...
	SOUND_set_location(gems_sound[index], &objmesh->location, objmesh->radius * gem_factor);
...
void load_level(void)
{
    ...
    level_clear->btrigidbody->setCollisionFlags(level_clear->btrigidbody->getCollisionFlags() | btCollisionObject::CF_CUSTOM_MATERIAL_CALLBACK | btCollisionObject::CF_NO_CONTACT_RESPONSE);
    ...
    FONT_load(font_small, font_small->name, 1, 24.0f, 512, 512, 32, 96);
    font_big = FONT_init((char *)"foo.ttf");
    FONT_load(font_big, font_big->name, 1, 48.0f, 512, 512, 32, 96);
    MEMORY *memory = NULL;
    i = 0;
    while (i != 4) {
	switch (i) {
    ...
    SOUND_set_location(water_sound, &objmesh->location, objmesh->radius);
    ...
    SOUND_set_location(lava_sound, &objmesh->location, objmesh->radius);
    ...
    SOUND_set_location(toxic_sound, &objmesh->location, objmesh->radius);
...
void templateAppDraw(void)
{
    ...
    GFX_load_identity();
    GFX_set_perspective(80.0f, (float)viewport_matrix[2] / (float)viewport_matrix[3], 0.1f, 50.0f, -90.0f);
    GFX_set_matrix_mode(MODELVIEW_MATRIX);
    GFX_load_identity();
    ...
    next_eye.y = player->location.y - distance * cosf(rotx * DEG_TO_RAD) * cosf(rotz * DEG_TO_RAD);
    next_eye.z = player->location.z + distance * sinf(rotx * DEG_TO_RAD);
    player->location.z += player->dimension.z;
    btVector3 p1(player->location.x, player->location.y, player->location.z), p2(next_eye.x, next_eye.y, next_eye.z);
    ClosestNotMeRayResultCallback back_ray(player->btrigidbody, p1, p2);
    dynamicsworld->rayTest(p1, p2, back_ray);
    if (back_ray.hasHit()) {
	back_ray.m_hitNormalWorld.normalize();
	next_eye.x = back_ray.m_hitPointWorld.x() + (back_ray.m_hitNormalWorld.x() * 0.1f);
	next_eye.y = back_ray.m_hitPointWorld.y() + (back_ray.m_hitNormalWorld.y() * 0.1f);
	next_eye.z = back_ray.m_hitPointWorld.z() + (back_ray.m_hitNormalWorld.z() * 0.1f);
    }
    ...
    GFX_look_at(&eye, &player->location, &up);
    ...
    char gem_str[MAX_CHAR] = { "" }, time_str[MAX_CHAR] = {
    ""}, level_str[MAX_CHAR] = {
    ""};
    if (game_state) {
	sprintf(level_str, "Level Clear!");
	FONT_print(font_big, viewport_matrix[3] * 0.5f - FONT_length(font_big, level_str) * 0.5f + 4.0f, viewport_matrix[2] - font_big->font_size * 1.5f - 4.0f, level_str, &font_color);
	/* Yellow. */
	font_color.x = 1.0f;
	font_color.y = 1.0f;
	font_color.z = 0.0f;
	FONT_print(font_big, viewport_matrix[3] * 0.5f - FONT_length(font_big, level_str) * 0.5f, viewport_matrix[2] - font_big->font_size * 1.5f, level_str, &font_color);
    }
    font_color.x = 0.0f;
    font_color.y = 0.0f;
    font_color.z = 0.0f;
    ...
    FONT_print(font_small, viewport_matrix[3] - FONT_length(font_small, gem_str) - 6.0f, (font_small->font_size * 0.5f), gem_str, &font_color);
    FONT_print(font_small, 8.0f, (font_small->font_size * 0.5f), time_str, &font_color);
    font_color.x = 1.0f;
    font_color.y = 1.0f;
    font_color.z = 0.0f;
    FONT_print(font_small, viewport_matrix[3] - FONT_length(font_small, gem_str) - 8.0f, (font_small->font_size * 0.5f), gem_str, &font_color);
    FONT_print(font_small, 6.0f, (font_small->font_size * 0.5f), time_str, &font_color);
    if (!game_state)
	game_time += SOUND_get_time(background_sound);
}

void templateAppToucheBegan(float x, float y, unsigned int tap_count)
{
    if (game_state == 1 && tap_count >= 2)
	game_state = 2;
}
...
{
    current_methods.enable_accelerometer();
