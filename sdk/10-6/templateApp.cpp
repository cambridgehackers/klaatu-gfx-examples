...
void program_draw(void *ptr)
{
	...
	glUniform4fv(PROGRAM_get_uniform_location(program, tmp), 1, (float *)&lamp[i]->color);
	vec4 position;
	sprintf(tmp, "LAMP_VS[%d].position", i);
	LAMP_get_position_in_eye_space(lamp[i], &gfx.modelview_matrix[gfx.modelview_matrix_index - 1], &position);
	glUniform3fv(PROGRAM_get_uniform_location(program, tmp), 1, (float *)&position);
	sprintf(tmp, "LAMP_FS[%d].distance", i);
	glUniform1f(PROGRAM_get_uniform_location(program, tmp), lamp[i]->distance);
	++i;
    }
}
