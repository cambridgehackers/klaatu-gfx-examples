...
void templateAppDraw(void)
{
	...
	camera->btrigidbody->setLinearVelocity(btVector3(forward.x * move_delta.z * 6.7f, forward.y * move_delta.z * 6.7f, 0.0f));
	camera->btrigidbody->setActivationState(ACTIVE_TAG);
    } else {
	camera->btrigidbody->setActivationState(ISLAND_SLEEPING);
    }
    GFX_translate(camera->location.x, camera->location.y, camera->location.z + (camera->dimension.z * 0.5f));
