...
unsigned char restart_game = 0, momo_launch = 0, banana = 0;
vec3 eye = { 3.5f, -10.8f, 5.3f }, center = {
3.5f, -9.8f, 5.3f}, up = {
0.0f, 0.0f, 1.0f};
			    ...
			    const btCollisionObject * btcollisionobject0, int part_0, int index_0, const btCollisionObject * btcollisionobject1, int part_1, int index_1)
{
    ...
    if ((strstr(objmesh0->name, "momo") || strstr(objmesh1->name, "momo"))
	&& (strstr(objmesh0->name, "banana") || strstr(objmesh1->name, "banana"))) {
...
void templateAppToucheEnded(float x, float y, unsigned int tap_count)
{
	...
	momo->btrigidbody->setLinearVelocity(btVector3(CLAMP((y - start_pos.y) * 0.1f, 0.0f, 10.0f), 0.0f, CLAMP((x - start_pos.x) * 0.1f, 0.0f, 10.0f)));
    }
}
