...
vec3 eye, next_eye, center = { 0.0f, 0.0f, 0.0f }
...
{
    if ((player->btrigidbody == btbroadphasepair.m_pProxy0->m_clientObject || player->btrigidbody == btbroadphasepair.m_pProxy1->m_clientObject)
...
void load_game(void)
{
    ...
    navigationpathdata_player.path_point_count = navigationpathdata_enemy.path_point_count = 0;
    player_next_point = enemy_next_point = -1;
...
void templateAppDraw(void)
{
    ...
    GFX_load_identity();
    GFX_set_perspective(80.0f, (float)viewport_matrix[2] / (float)viewport_matrix[3], 1.0f, 1000.0f, -90.0f);
    if (game_over == 2) {
	templateAppExit();
	load_game();
	game_over = 0;
	...
	NAVIGATION_get_path(navigation, &navigationpath_enemy, &navigationpathdata_enemy);
	enemy_next_point = 1;
	start_time = get_milli_time();
    }
    if (navigationpathdata_enemy.path_point_count) {
	vec3 color = { 1.0f, 0.0f, 0.0f };
	move_entity(enemy, &navigationpathdata_enemy, &enemy_next_point, 4.0f);
	...
	if (!font) {
	    ...
	    FONT_load(font, font->name, 1, 64.0f, 512, 512, 32, 96);
	}
	float posx = (viewport_matrix[3] * 0.5f) - (FONT_length(font, msg) * 0.5f), posy = viewport_matrix[2] - font->font_size;
