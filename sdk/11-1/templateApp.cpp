...
unsigned int colorbuffer_texture = 0, colorbuffer_width = 128, colorbuffer_height = 256;
...
void program_draw(void *ptr)
{
	...
	// Material Data
		...
		glUniform4fv(program->uniform_array[i].location, 1, (float *)&black);
	    } else {
...
void templateAppInit(int width, int height)
{
	...
	OBJ_build_texture(obj, i, obj->texture_path, TEXTURE_MIPMAP | TEXTURE_16_BITS, TEXTURE_FILTER_3X, 0.0f);
	++i;
    }
    i = 0;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, colorbuffer_width, colorbuffer_height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, NULL);
...
void first_pass(void)
{
    ...
    draw_scene();
    glBindTexture(GL_TEXTURE_2D, colorbuffer_texture);
    glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, colorbuffer_width, colorbuffer_height);
}
...
void fullscreen_pass(void)
{
	...
	GFX_scale((float)viewport_matrix[2], (float)viewport_matrix[3], 1.0f);
	GFX_rotate(180.0f, 1.0f, 0.0f, 0.0f);
	PROGRAM_draw(program);
	vec2 radius = { blur_radius / (float)colorbuffer_width,
	    0.0f
	};
	glUniform2fv(PROGRAM_get_uniform_location(program, (char *)"BLUR_RADIUS"), 1, (float *)&radius);
	OBJ_draw_mesh(obj, OBJ_get_mesh_index(obj, "fullscreen", 0));
	radius.x = 0.0f;
